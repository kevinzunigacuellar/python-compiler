...
def compress_parameters(prog: Program) -> Program:
    ...
    def cp_function_def(function: FunctionDef) -> Tuple:
        ...
        return new_params, new_body_stmts

    def cp_function_call(call: Call):
        args = call.args
        uncompressed_args = args[0:compress_length-1]
        compressed_args = Prim("tuple", args[compress_length-1:])

        new_args = []
        for arg in uncompressed_args:
            new_args.append(arg)

        new_args.append(compressed_args)
        ...
        match expr:
            case Call(func_var, args):
                if len(args) > compress_length:
                    new_args = cp_function_call(expr)
                else:
                    new_args = args
        ...
        match stmt:
            case FunctionDef(name, params, body_stmts, return_type):
                if len(params) > compress_length:
                    new_params, new_body_stmts = cp_function_def(stmt)
                else:
                    new_params = params
                    new_body_stmts = body_stmts

                return FunctionDef(name, new_params, new_body_stmts, return_type)

            case Print(e):
                return Print(cp_expr(e))
            case Assign(x, e):
                return Assign(x, cp_expr(e))
            case _:
                return stmt
