...
for file_name in sorted(os.listdir('project/tests')):
    if file_name.endswith('.py'):
        with open('project/tests/' + file_name) as f:
            ...
            try:
                ...
                if run_gcc:
                    ...
                    with open(asm_file_name, 'w') as output_file:
                        output_file.write(x86_program)  # type: ignore
                    ...
                    binary_result = subprocess.run(
                        ["./a.out"], text=True, capture_output=True)
                    print('Binary output:', binary_result.stdout)

                    interpreter_result_str = '\n'.join(
                        [str(int(i)) for i in interpreter_result]) + '\n'
