...
def typecheck(program: Program) -> Program:
    ...
    """
    # this typecheck only supports integers and booleans
    def tc_exp(e: Expr, env: TEnv) -> type:
        match e:
            ...
            case Prim(op, args):
                match op:
                    case 'add' | 'sub':
                        assert tc_exp(args[0], env) == int
                        assert tc_exp(args[1], env) == int
                        return int
                    case 'or' | 'and':
                        assert tc_exp(args[0], env) == bool
                        assert tc_exp(args[1], env) == bool
                        return bool
                    case 'not':
                        assert tc_exp(args[0], env) == bool
                        return bool
                    case 'eq' | 'gt' | 'gte' | 'lt' | 'lte':
                        assert tc_exp(args[0], env) == tc_exp(args[1], env)
                        return bool
                    case _:
                        raise Exception(f'unknown op {op}', e)
    ...
    def rco_stmt(stmt: Stmt, bindings: Dict[str, Expr]) -> Stmt:
        match stmt:
            ...
            case Print(e1):
                new_e1 = rco_exp(e1, bindings)
                return Print(new_e1)
            case If(condition, then_stmt, else_stmt):
...
def explicate_control(prog: Program) -> cif.CProgram:
    ...
    def ec_expr(e:Expr) -> cif.Expr:
        match e:
            case Prim(op, args):
                return cif.Prim(op, [ec_atm(a) for a in args])
            case _:
                return ec_atm(e)

    def ec_stmt(stmt: Stmt, cont: List[cif.Stmt]) -> List[cif.Stmt]:
        match stmt:
            case Assign(x, e):
            ...
            case If(condition, then_stmt, else_stmt):
                ...
                then_label = create_block(ec_stmts(then_stmt, [cif.Goto(cont_label)]))
                else_label = create_block(ec_stmts(else_stmt, [cif.Goto(cont_label)]))
                new_stmt: List[cif.Stmt] = [cif.If(ec_expr(condition), cif.Goto(then_label), cif.Goto(else_label))]
                return new_stmt
            case _:
                raise Exception('ec_stmt', stmt)
    ...
    continuation = [cif.Return(cif.Constant(0))]
    basic_blocks['start'] = ec_stmts(prog.stmts, continuation)
...
def select_instructions(prog: cif.CProgram) -> x86.X86Program:
    ...
    def si_stmt(stmt: cif.Stmt) -> List[x86.Instr]:
        match stmt:
            case cif.Assign(x, cif.Prim(op, atms)):
                if op in op_instructions:
                    return [x86.NamedInstr('movq', [si_atm(atms[0]), x86.Reg('rax')]),
                            x86.NamedInstr(op_instructions[op], [si_atm(atms[1]), x86.Reg('rax')]),
                            x86.NamedInstr('movq', [x86.Reg('rax'), x86.Var(x)])]
                elif op in cmp_code:
                    return [x86.NamedInstr('cmpq', [si_atm(atms[1]), si_atm(atms[0])]),
                            x86.Set(cmp_code[op], x86.ByteReg('al')),
                            x86.NamedInstr('movzbq', [x86.ByteReg('al'), x86.Var(x)])]
                elif op == 'not':
                    return [x86.NamedInstr('movq', [si_atm(atms[0]), x86.Var(x)]),
            ...
            case cif.If(atm1, cif.Goto(label1), cif.Goto(label2)):
                return [x86.NamedInstr('cmpq', [si_atm(atm1),x86.Immediate(1)]),
                        x86.JmpIf('e', label1),
                        x86.Jmp(label2)]
            case cif.Print(atm1):
            ...
            case cif.Return(atm1):
                return [x86.NamedInstr('movq', [si_atm(atm1), x86.Reg('rax')]),
                        x86.Jmp('conclusion')]
            case cif.Goto(label1):
                return [x86.Jmp(label1)]
            case _:
                raise Exception('si_stmt', stmt)
...
def allocate_registers(program: x86.X86Program) -> x86.X86Program:
    ...
    def reads_of(i: x86.Instr) -> Set[x86.Var]:
        match i:
            ...
            case x86.NamedInstr('cmpq', [e1, e2]): # this may be wrong
                return vars_arg(e1).union(vars_arg(e2))
            case x86.NamedInstr('andq', [e1, e2]):
                return vars_arg(e1).union(vars_arg(e2))
    ...
    # save the live-after sets in the global dict (x)
    # save the live-before set in the global dict (x)
    def ul_block(label: str):
        current_live_after: Set[x86.Var] = set()
        local_live_after_sets: List[set] = []

        program_block = program.blocks[label] if label in program.blocks else []

        for i in reversed(program_block):
            local_live_after_sets.append(current_live_after)
            current_live_after = ul_instr(i, current_live_after)

        # TODO: for label 'conclusion', add the return value to the live-before set
    ...
    def ah_arg(a: x86.Arg) -> x86.Arg:
        match a:
            ...
            case x86.Var(x):
                return homes[a]
            case x86.ByteReg(r): # maybe this is wrong
                return a
            case _:
    ...
    def ah_instr(e: x86.Instr) -> x86.Instr:
        match e:
            ...
            case _:
                if isinstance(e, (x86.Callq, x86.Retq, x86.Jmp, x86.Set, x86.JmpIf)): # this maybe wrong
                    return e
                else:
...
def patch_instructions(program: x86.X86Program) -> x86.X86Program:
    ...
    def pi_instr(e: x86.Instr) -> List[x86.Instr]:
        match e:
            ...
            case _:
                if isinstance(e, (x86.Callq, x86.Retq, x86.Jmp, x86.NamedInstr, x86.Set, x86.JmpIf)):
                    return [e]
                else:
