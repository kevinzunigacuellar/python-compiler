...
def typecheck(program: Program) -> Program:
    ...
    def tc_exp(e: Expr, env: TEnv) -> type:
        match e:
            ...
            # This will support (1,2)[0] but compute types twice
            case Prim('subscript', [e1, Constant(idx)]):
                t_e1 = tc_exp(e1, env)
                assert type(t_e1) == tuple
                return t_e1[idx]
            
            # case Prim('subscript', [Var(a), Constant(idx)]):
            #     tuple_types = env[a]
            #     assert type(tuple_types) == tuple
            #     return tuple_types[idx]
                
            case Prim('tuple', args):
                types = []
                for arg in args:
                    types.append(tc_exp(arg, env))
                return tuple(types)
...
def expose_alloc(prog: Program) -> Program:
    ...
    def make_tag(ts):
        ...
        for t in ts:
            ...
            if isinstance(t, tuple):
                ...
                tag = tag + 0
        # 2. construct the length
        tag = tag << 6
        tag = tag + len(ts)
        # 3. construct the forwarding pointer indicator
        tag = tag << 1
        tag = tag + 1
    ...
    # x = tuple(args)
    # this is true because we've already run rco
    def ea_stmt(s: Stmt) -> List[Stmt]:
        match s:
            ...
            case If(c_expr, then_stmts, else_stmts):
                return [If(c_expr, ea_stmts(then_stmts), ea_stmts(else_stmts))]
            case _:
                return [s]
    def ea_stmts(stmts):
        new_stmts = []
...
def select_instructions(prog: ctup.CProgram) -> x86.X86Program:
    ...
    def si_stmt(stmt: ctup.Stmt) -> List[x86.Instr]:
        match stmt:
            # TODO: add cases for tuple, subscript, allocate, collect, tuple_set
...
def run_compiler(s, logging=True):
