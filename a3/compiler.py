...
def allocate_registers(prog: x86.X86Program) -> x86.X86Program:
    """
    ...
    def ul_block(instrs: List[x86.Instr]) -> List[Set[x86.Var]]:
        ...
        # reverse returns a iterator, so we need to convert it to a list
        return list(reversed(live_after_sets))

    # --------------------------------------------------
    # interference graph
    # --------------------------------------------------
    def bi_instr(i: x86.Instr, live_after: Set[x86.Var], graph: InterferenceGraph, move_grah: InterferenceGraph):
        w_vars = writes_of(i)

        # add move edges to move_graph
        match i:
            case x86.NamedInstr("movq", [a1, a2]):
                if isinstance(a1, x86.Var) and isinstance(a2, x86.Var):
                    # add edges in both directions
                    move_graph.add_edge(a1, a2)
                    move_graph.add_edge(a2, a1)

        for w_var in w_vars:
            for life_var in live_after:
                # add edges in both directions
                graph.add_edge(w_var, life_var)
                graph.add_edge(life_var, w_var)

    def bi_block(instrs: List[x86.Instr], live_afters: List[Set[x86.Var]], graph: InterferenceGraph, move_grah: InterferenceGraph):
        for instr, live_var in zip(instrs, live_afters):
            bi_instr(instr, live_var, graph, move_grah)

    # --------------------------------------------------
    # graph coloring
    # --------------------------------------------------
    def color_graph(local_vars: Set[x86.Var], interference_graph: InterferenceGraph, move_graph: InterferenceGraph) -> Coloring:
        saturation_sets: Dict[x86.Var, Saturation] = {}

        def find_most_saturated_var() -> x86.Var:

            # record the length of the saturation sets for each remaining variable
            saturation_lens = {
                var: len(saturation_sets[var]) for var in vars_to_color
            }
            # get the highest value of saturation_lens
            max_saturation_len = max(saturation_lens.values())
            
            # get all vars with the max saturation len
            multiple_maxes = [k for k, v in saturation_lens.items() if v == max_saturation_len]

           # if no tie
            if len(multiple_maxes) == 1:
                return multiple_maxes[0]
            else:
                for var in multiple_maxes:
                    # check if var has any neighbors
                    if move_graph.neighbors(var):
                        # check if any of its neighbors have been colored
                        for neighbor in move_graph.neighbors(var):
                            if neighbor in coloring.keys():
                                # if so, break the tie by choosing that var
                                return var
                
                # if tie cannot be broken, choose the first var 
                # This is the case for the first iteration
                return multiple_maxes[0]

        # initialize saturation empty sets
        ...
        while len(vars_to_color) > 0:
            # find the most saturated variable that has not been colored
            x = find_most_saturated_var()
                
            # pick the lowest color not in its saturation set
            x_color = min(set(range(0, len(local_vars))).difference(saturation_sets[x]))

            # update coloring
            coloring[x] = x_color

            # update saturation sets
            for y in interference_graph.neighbors(x):
                if isinstance(y, x86.Var):
                    saturation_sets[y].add(x_color)

            # update copy
            vars_to_color.discard(x)
    ...
    blocks = prog.blocks
    instrs = []
    for block_instrs in blocks.values():
        instrs.extend(block_instrs)

    live_after_sets = ul_block(instrs)
    ...
    # Build a move graph
    move_graph = InterferenceGraph()
    
    bi_block(instrs, live_after_sets, interference_graph, move_graph)

    log_ast('interference graph', interference_graph)
    log_ast('move graph', move_graph)

    # Step 3: Color the graph
    coloring = color_graph(all_vars, interference_graph, move_graph)
    ...
    for color in set(coloring.values()):
        if len(available_registers) > 0:
            register_to_set = available_registers[0]
            available_registers.remove(register_to_set)
            color_map[color] = x86.Reg(register_to_set)
        else:
            offset = -8 * (stack_locations_used + 1)
            deref = x86.Deref("rbp", offset)
            color_map[color] = deref
            stack_locations_used += 1

    # Step 4.2: Compose the "coloring" with the "color map" to get "homes"
    for var in all_vars:
        homes[var] = color_map[coloring[var]]
    ...
    stack_size = align(8 * stack_locations_used)
